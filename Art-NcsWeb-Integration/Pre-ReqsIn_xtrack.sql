...
IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = 'target')
    EXEC('CREATE SCHEMA [target]');

IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = 'art')
...
alter table tblStayPayor
add StateId int null

alter table tblFacDistinctUnit
add SyncId int null

ALTER TABLE [tblAging.Bill]
ADD Deleted bit NULL CONSTRAINT DF_tblAging_Bill_Deleted DEFAULT (0);

ALTER TABLE [tblAging.Cr]
ADD Deleted bit NULL CONSTRAINT DF_tblAging_Cr_Deleted DEFAULT (0);
...
--check if this will be a problem for the current integration
update tblPayor
set name = 'Private Pay'
where FriendlyName = 'Private'
and LevelId = 5


update tblPayor
set name = 'HMO/Private Insurance'
where FriendlyName = 'HMO'
and LevelId = 5

go

--add StateId to tblStayPayors
CREATE OR ALTER PROCEDURE [dbo].[GenerateMergeStatement] 
...
BEGIN
    ...
    IF @DeltaMode = 1 
       AND (@DeleteNotMatchedBySource = 1 OR @SoftDeleteNotMatchedBySource = 1)
    BEGIN
        RAISERROR('DeltaMode cannot use DeleteNotMatchedBySource or SoftDeleteNotMatchedBySource because the source is not a full dataset.', 16, 1);
        RETURN;
    END;
    ...
    SELECT @JoinCondition = 
        STUFF((
            SELECT 
                ' AND ' +
                CASE 
                    WHEN tgt.DATA_TYPE IN ('char', 'varchar', 'nchar', 'nvarchar', 'text', 'ntext')
                     AND src.DATA_TYPE IN ('char', 'varchar', 'nchar', 'nvarchar', 'text', 'ntext')
                        THEN 'tgt.' + QUOTENAME(LTRIM(RTRIM(pk.value))) + ' COLLATE DATABASE_DEFAULT = src.' + QUOTENAME(LTRIM(RTRIM(pk.value))) + ' COLLATE DATABASE_DEFAULT'
                    ELSE 'tgt.' + QUOTENAME(LTRIM(RTRIM(pk.value))) + ' = src.' + QUOTENAME(LTRIM(RTRIM(pk.value)))
                END
            FROM STRING_SPLIT(@PrimaryKeyColumns, ',') pk
            JOIN INFORMATION_SCHEMA.COLUMNS tgt
                ON tgt.TABLE_SCHEMA = @TargetSchema
               AND tgt.TABLE_NAME = @TargetTable
               AND tgt.COLUMN_NAME = LTRIM(RTRIM(pk.value))
            JOIN INFORMATION_SCHEMA.COLUMNS src
                ON src.TABLE_SCHEMA = @SourceSchema
               AND src.TABLE_NAME = @SourceTable
               AND src.COLUMN_NAME = LTRIM(RTRIM(pk.value))
            FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 5, '');

    -------------------------------------------------
    -- Get column names dynamically
    -- SOURCE TABLE IS THE DRIVER
    -------------------------------------------------
    SELECT @Columns = 
        STUFF((
                ...
                SELECT 
                    ...
                    CASE 
                        WHEN c.DATA_TYPE IN ('char', 'varchar', 'nchar', 'nvarchar', 'text', 'ntext')
                            THEN '(' +
                                 'tgt.' + QUOTENAME(c.COLUMN_NAME) + ' COLLATE DATABASE_DEFAULT <> src.' + QUOTENAME(c.COLUMN_NAME) + ' COLLATE DATABASE_DEFAULT' +
                                 ' OR (tgt.' + QUOTENAME(c.COLUMN_NAME) + ' IS NULL AND src.' + QUOTENAME(c.COLUMN_NAME) + ' IS NOT NULL)' +
                                 ' OR (tgt.' + QUOTENAME(c.COLUMN_NAME) + ' IS NOT NULL AND src.' + QUOTENAME(c.COLUMN_NAME) + ' IS NULL)' +
                                 ')'
                        ELSE '(' +
